###################
#Función pura x**2
###################
def cuad(x):
    return x*x

###################
#Función pura x**3
###################
def cub(x):
    return x*x*x

############################
#Mapeo de una funcion pura
############################
def mapeo(func,lista_numeros):
    resultado=[]

    for i in lista_numeros:
        resultado.append(func(i))
    return resultado

cuadrados = mapeo(cuad,[2.5,2,3.8,1.2,6.6,1j,7,8])
cubos=mapeo(cub,[1,2,3,4,5,6,7,8])
print(cuadrados)
print(cubos)

##############################
# Funcion dentro de funciones
##############################
def en_mayusculas(texto):
    return texto.upper()
def en_minusculas(texto):
    return texto.lower()
def saludar(func):
    saludo = func("Hola, ¿Qué tal")
    print(saludo)

###############
# Con Strings
###############
saludar(en_mayusculas)
saludar(en_minusculas)

##########################################
#Funciones abstractas dentro de funciones
#Su resultado es otra función
#########################################
def divisor(x):
    def dividendo(y):
            return y/x
    return dividendo

##################################
#Primero generamos la función y/2
##################################
división = divisor(2)

#############################
#La usamos para calcular 3/2
#############################
print (división(3))

######################################
#Uso de la función map con una lista
######################################

#########################################
#Lista de las ciudades y su temperatura
#########################################

temps= [("Berlin",29),("Cairo",36),("Buenos Aires",19),("Los Angeles",26),
        ("Tokyo",27),("Nueva York",28),("Londres",22),("Infierno",32),("Tenoctitlan", 23)]

C_A_F = lambda datos: (datos[0], (9/5)*datos[1] + 32)
print(list(map(C_A_F,temps)))

